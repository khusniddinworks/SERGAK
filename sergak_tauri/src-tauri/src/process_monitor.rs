...
pub fn get_process_tree() -> Vec<ProcessNode> {
    ...
    for (pid, process) in sys.processes() {
        ...
        if ["cmd.exe", "powershell.exe", "pwsh.exe", "wscript.exe", "cscript.exe", "mshta.exe"].contains(&name_lower.as_str()) {
            if let Some(parent_pid) = ppid {
                if let Some(parent) = sys.process(sysinfo::Pid::from_u32(parent_pid)) {
                    ...
                    if ["winword.exe", "excel.exe", "powerpnt.exe", "outlook.exe", "chrome.exe", "msedge.exe"].contains(&parent_name.as_str()) {
                        is_suspicious = true;
